...
export default function Cart() {
  ...
  // Memoized function to increment quantity
  const increment = useCallback((id) => {
    setQuantities((prev) => ({ ...prev, [id]: prev[id] + 1 }));
  }, []);

  // Memoized function to decrement quantity
  ...
  return (
    <div className="max-w-[1200px] mx-auto my-5 bg-white rounded-lg shadow-lg p-6">
      <div className=" h-full w-full rounded-lg ">
        ...
        {carts.length === 0 ? (
          <div className="h-[50%] text-black flex justify-center items-center font-medium text-4xl">
          ...
          <>
            <div className="grid grid-cols-3 gap-4 p-5 bg-white">
              {carts.map((card) => (
                <ProductsCart
                  ...
                  increment={() => increment(card.id)}
                  decrement={() => decrement(card.id)}
                />
              ))}
            </div>
            <hr></hr>
            <div className="flex flex-col w-full items-end">
              ...
              <button
                type="button"
                className="bg-[#ef6027] text-white px-4 py-2 font-semibold rounded-full "
              >
                Check Out
              </button>
            </div>
